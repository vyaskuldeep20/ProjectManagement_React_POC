...
server.use((req, res, next) => {
  var today=new Date();
  if (req.method === "POST") {
    req.body.createdAt = String(today.getDate()).padStart(2, '0')+"-"+String(today.getUTCMonth()+1).padStart(2, '0')+"-"+today.getUTCFullYear();
  }
...
function validateProject(project) {
  ...
  if (!project.createdAt) return "creation date is required.";
  return "";
}
...
function validateUser(user) {
  ...
  if (!user.createdAt) return "Creation date is required.";
  return "";
}

function validateTask(project) {
  if (!project.name) return "Task name is required.";
  if (!project.detail) return "Task detail is required.";
  if (!project.user) return "User info is required.";
  if (!project.status) return "Project Status is required.";
  if (!project.createdAt) return "Creation date is required.";
  return "";
}
